...
g_24 = np.zeros(len(Q_vals),dtype=complex)
for Q_index in range(len(Q_vals)):
    g_24[Q_index] = g_ij(2,4,c6,r_index,Q_index,0)

g_old = np.load(f'/home/velni/phd/w/tfm/py/metric/data/0/g_24_r={r_index}.npy')

for Q in range(len(Q_vals)):
    if g_24[Q] == g_old[Q]:
        pass
    else:
        print(f'ERRO!: g={g_24[Q]} pero g_old={g_old[Q]}')
        break
