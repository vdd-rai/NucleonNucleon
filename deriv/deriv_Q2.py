...
def D7U_p_mass(coord,r_index,Q1,Q2p,hD):
    ...
    # Q2 = np.dot(Q2p, np.exp(-1j*0.5*hD*sigma[0]))
    x = -1j*0.5*hD*sigma[0]
...
def D8U_p_mass(coord,r_index,Q1,Q2p,hD):
    ...
    # Q2 = np.dot(Q2p, np.exp(-1j*0.5*hD*sigma[1]))
    x = -1j*0.5*hD*sigma[1]
    ex = I + x + 0.5*np.dot(x,x) + (1./6)*np.dot(x,np.dot(x,x))
    Q2 = np.dot(Q2p,ex)
...
def D9U_p_mass(coord,r_index,Q1,Q2p,hD):
    ...
    # Q2 = np.dot(Q2p, np.exp(-1j*0.5*hD*sigma[2]))
    x = -1j*0.5*hD*sigma[2]
    ex = I + x + 0.5*np.dot(x,x) + (1./6)*np.dot(x,np.dot(x,x))
    Q2 = np.dot(Q2p,ex)
...
for r_index in range(len(r_vals)):
    # np.save(f'/home/velni/Escritorio/TFM/py/deriv/data/m/D7/D7_U_mass_r={r_index}_Q={Q_index}', D7_U_mass(r_index,I,Q_vals[Q_index],hD7))
    # np.save(f'/home/velni/Escritorio/TFM/py/deriv/data/m/D8/D8_U_mass_r={r_index}_Q={Q_index}', D8_U_mass(r_index,I,Q_vals[Q_index],hD8))
    np.save(f'/home/velni/Escritorio/TFM/py/deriv/data/m/D9/D9_U_mass_r={r_index}_Q={Q_index}', D9_U_mass(r_index,I,Q_vals[Q_index],hD9))
...
U = np.load('/home/velni/Escritorio/TFM/py/deriv/data/m/D7/D7_U_mass_r=23_Q=0.npy')
