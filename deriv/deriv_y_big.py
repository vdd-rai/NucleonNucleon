...
def d1U(r_index,Q_index,mod,hD):
	...
	U_vals = np.load(f'/home/velni/phd/w/tfm/py/prod/data/{mod}/U_S_r={r_index}_Q={Q_index}.npy')
...
for i,point in enumerate(point_list):
    t0 = time.time()
    #np.save(f'/home/velni/phd/w/tfm/py/deriv/data/0/d1/d1_U_r={point[0]}_Q={point[1]}', d1U(point[0],point[1],0,hD))
    #np.save(f'/home/velni/phd/w/tfm/py/deriv/data/0/d2/d2_U_r={point[0]}_Q={point[1]}', d2U(point[0],point[1],0,hD))
    np.save(f'/home/velni/phd/w/tfm/py/deriv/data/0/d3/d3_U_r={point[0]}_Q={point[1]}', d3U(point[0],point[1],0,hD))
    stept = time.time() - t0
    print(f'done: {i+1}/{len(point_list)} ----- eta: {stept*(len(point_list)-i-1)/60.} min')
