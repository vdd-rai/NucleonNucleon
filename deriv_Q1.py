...
@njit
def D4U_p_mass(coord,r_index,Q1p,Q2,hD):
...
@njit
def D4U_m_mass(coord,r_index,Q1p,Q2,hD):
    """ Compute product approximation in -hD displaced
    """
...
def D4_U_mass(r_index,Q1p,Q2,hD):
    D4U_vals = np.zeros((len(y1),len(y2),len(y3),4))
    for idx in idx_list:
        D4U_vals[idx[0]][idx[1]][idx[2]] = (1./(2*hD))*( D4U_p_mass(idx,r_index,Q1p,Q2,hD) - D4U_m_mass(idx,r_index,Q1p,Q2,hD) )
    return D4U_vals

# D5

@njit
def D5U_p_mass(coord,r_index,Q1p,Q2,hD):
    """ Compute product approximation in +hD displaced
    """
    ...
    x = -1j*0.5*hD*sigma[1]
    ex = I + x + 0.5*np.dot(x,x) + (1./6)*np.dot(x,np.dot(x,x))
    Q1 = np.dot(I,ex)
...
@njit
def D5U_m_mass(coord,r_index,Q1p,Q2,hD):
    """ Compute product approximation in -hD displaced
    """
...
def D5_U_mass(r_index,Q1p,Q2,hD):
    D5U_vals = np.zeros((len(y1),len(y2),len(y3),4))
    for idx in idx_list:
        D5U_vals[idx[0]][idx[1]][idx[2]] = (1./(2*hD))*( D5U_p_mass(idx,r_index,Q1p,Q2,hD) - D5U_m_mass(idx,r_index,Q1p,Q2,hD) )
    return D5U_vals

# D6

@njit
def D6U_p_mass(coord,r_index,Q1p,Q2,hD):
    """ Compute product approximation in +hD displaced
    """
...
@njit
def D6U_m_mass(coord,r_index,Q1p,Q2,hD):
    """ Compute product approximation in -hD displaced
    """
...
def D6_U_mass(r_index,Q1p,Q2,hD):
    D6U_vals = np.zeros((len(y1),len(y2),len(y3),4))
    for idx in idx_list:
        D6U_vals[idx[0]][idx[1]][idx[2]] = (1./(2*hD))*( D6U_p_mass(idx,r_index,Q1p,Q2,hD) - D6U_m_mass(idx,r_index,Q1p,Q2,hD) )
    return D6U_vals
...
U = D6_U_mass(r_index,I,Q_vals[Q_index],hD)
...
for r_index in range(len(r_vals)):
    np.save(f'/home/velni/Escritorio/TFM/py/deriv/data/m/D4/D4_U_mass_r={r_index}_Q={Q_index}', D4_U_mass(r_index,I,Q_vals[Q_index],hD4))
    np.save(f'/home/velni/Escritorio/TFM/py/deriv/data/m/D5/D5_U_mass_r={r_index}_Q={Q_index}', D5_U_mass(r_index,I,Q_vals[Q_index],hD5))
    np.save(f'/home/velni/Escritorio/TFM/py/deriv/data/m/D6/D6_U_mass_r={r_index}_Q={Q_index}', D6_U_mass(r_index,I,Q_vals[Q_index],hD6))
...
U = np.load('/home/velni/Escritorio/TFM/py/deriv/data/m/D4/D4_U_mass_r=23_Q=0.npy')
