...
for c_idx,c in enumerate(uvw_str):
    for i in range(4):
        if c[i] == str('1'):
            comb = i+1
            sgn = 0
    ...
    dQ_vals[k].append( dQ_an[k].evalf(subs={u:uvw[c_idx][xu],v:uvw[c_idx][xv],w:uvw[c_idx][xw]}) )

dQ_export = abs(float(dQ_vals[0][0]))*np.ones(32)

#%% save data

np.save('/home/velni/phd/w/tfm/py/sample/dQ.npy', dQ_export)
